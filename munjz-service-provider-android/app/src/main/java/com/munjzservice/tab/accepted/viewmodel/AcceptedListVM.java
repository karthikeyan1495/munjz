...
public class AcceptedListVM extends java.util.Observable{
    ...
    public AcceptedListVM(@NonNull Activity activity){
    ...
    private void callRequestService(int page){
        if (InternetReachability.getInstance().isReachable()) {
            ...
            APICall api = APIConfiguration.getInstance().createService
                    (APICall.class);
            Observable<Response<ActiveModel>> observable = api.requestList(AppSession.getInstance(activity).getCustomerInfo().getUserid(),"acceptservice",page);
